...
def get_all_links():
    ArchiveList = pd.read_excel("RR Private Owners.xlsx")
    links = []
...
def main():
    ...
    for link in links:
        ...
        for i in ["&Page=2","&Page=3","&Page=4","&Page=5","&Page=6","&Page=7"]:
            try:
        ...
        folder_path = 'RR Pictures Archive/'
        full_path = os.path.join(folder_path, csv_name)
        table_data.to_csv(full_path, index=False)
        print(table_data)
    #     if table_data:
    #         if headers is None:
